 ...
 console.log(doctors);
  ...
  <CommandList className="overflow-visible">
    ...
    <CommandItem key={index}>
        <Link href={'/search/'+item.label} className={`p-2 flex gap-2 
        items-center
        text-blue-600
        font-bold
        rounded-md
        cursor-pointer
        w-full
        ${paramsCate == item.label && 'bg-blue-100'}
        `}> 
        <img src={item.src} alt={`Image ${index + 1}`} className="w-10 h-10 object-cover" />
